...
- **Memory Leak Warning (Issue #12)**: Fixed "possible EventEmitter memory leak detected" warning that appeared when more than 10 clients connected. Now properly calls `setMaxListeners()` with the default value of 0 (unlimited), which disables the warning while still allowing unlimited connections.
...
- **Memory Leak Warning (Issue #12)**: Fixed "possible EventEmitter memory leak detected" warning by always calling `setMaxListeners()`. The default config sets maxListeners to 0 (unlimited), which now properly disables the warning. Users can still set a specific limit via options.
